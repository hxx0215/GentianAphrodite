...
import { async_eval } from '../../vendors/async-eval.ts'
...
const LongTermMemories: LongtermMemory[] = loadJsonFileIfExists(path.join(chardir, 'memory/long-term-memory.json'), [])
...
async function runLongTermMemoryTrigger(memory: LongtermMemory, args: Channel, logical_results: LogicalResult) {
...
export async function testLongTermMemoryTrigger(memory: LongtermMemory, args: Channel, logical_results: LogicalResult) {
 ...
 */
export function getLongTermMemoryByName(name: string) {
...
export function formatLongTermMemoryContext(memory: LongtermMemory) {
	if (!memory) return '找不到指定的记忆。'
	const context_parts = Object.entries(context_prompt_build_table).map(([key, prefix]) => {
		const value = memory[key as keyof LongtermMemory]
...
export function formatLongTermMemory(memory: LongtermMemory) {
...
export async function LongTermMemoryPrompt(args: Channel, logical_results: LogicalResult) : Promise<Prompt>{
	const actived_memories: LongtermMemory[] = []
	for (const memory of LongTermMemories)
...
`
	...
	if (args.extension?.enable_prompts?.LongTermMemory || !logical_results.in_assist || await match_keys(args as any, ['记忆', '记住'], 'user'))
...
export function addLongTermMemory(memory: LongtermMemory) {
 ...
 */
export function updateLongTermMemory({ name, trigger, prompt, updatedAt, updatedContext }: LongtermMemory) {
...
export function deleteLongTermMemory(name: string) {
...
export function getRandomNLongTermMemories(n: number) {
